...
```{r}
covid_data_simple |> 
  filter(date %in% c("2020-04-12","2020-04-15"))|>
ggplot(aes(x = date, y = Tests, color = race)) + geom_point()
`````````
...
covid_data_count |> 
  filter(state == "MA") |> 
ggplot(aes(x = population, y = Cases, color = race, shape = race)) + geom_point() 
`````````
...
The code given allows us to see a 1) Monthly view of total cases and 2) Monthly view of total cases labeled by race. Making some alterations to the code and looking at the table data, I was able to make a table that shows a daily view of tests taken organized by race.
...
Age might be a good datapoint as it tells us moer about the demographics; particularly the demographics within specific races. 